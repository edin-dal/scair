...
class DlamDeBruijnTests extends AnyFlatSpec:
   ...
   */
  "A polymorphic identity (de Bruijn) IR" should "build, verify, and print the expected shape" in {
    //
    // Types:
    ...
    vLambda.verify().isRight shouldBe true
    ...
    tRet.verify().isRight shouldBe true
    ...
    tLambda.verify().isRight shouldBe true
  ...
  "Nested tlambda + tapply (de Bruijn) IR" should "instantiate ∀ with bvar(IntData(0)) and type-check" in {
    ...
    inner_vLambda.verify().isRight shouldBe true
    ...
    inner_tLambda.verify().isRight shouldBe true
    ...
    outer_tLambda.verify().isRight shouldBe true

    val module = ModuleOp(
      Region(Seq(Block(operations = Seq(outer_tLambda))))
    )
    module.verify().isRight shouldBe true
  ...
  "Instantiate to a ground type (i32)" should "compute α→α @ α=i32 == i32→i32" in {
    ...
    tapp.verify().isRight shouldBe true
  }
  ...
  "TApply under an outer binder" should "instantiate ∀β. (α -> α) to (α -> α) where α is outer bvar(IntData(0))" in {
    ...
    innerVLam.verify().isRight shouldBe true
    ...
    val outerTRes = Result[TypeAttribute](forall(tapplyResultTy)) // ∀α. α -> α
    val outerTRegion =
      Region(
        Seq(
          Block(
  ...
  "Monomorphize pass" should "replace tapply with a specialized vlam and remove the use" in {
    ...
    G.verify().isRight shouldBe true
    ...
    F.verify().isRight shouldBe true
    ...
    afterMod.verify().isRight shouldBe true
    ...
    val outerAfter: TLambda =
      afterMod.regions.head.blocks.head.operations
        .collectFirst { case t: TLambda =>
          t
        }
    ...
    val spec = outerBlock.operations
      .collectFirst {
  ...
  "NatExpr printing" should "print literals and composite forms" in {
    import scair.dialects.dlam_de_bruijn.*

    val two = DlamNatLit(IntData(2))
    val three = DlamNatLit(IntData(3))
    val add = DlamNatAdd(two, three) // 2 + 3
    val mul = DlamNatMul(add, two) // (2 + 3) * 2

    printed(two) shouldBe "!dlam.nat_lit<2>"
    printed(add) shouldBe "!dlam.nat.add<!dlam.nat_lit<2>, !dlam.nat_lit<3>>"
    printed(
      mul
    ) shouldBe "!dlam.nat.mul<!dlam.nat.add<!dlam.nat_lit<2>, !dlam.nat_lit<3>>, !dlam.nat_lit<2>>"

  }

  "NatExpr helpers" should "build the same structures as direct constructors" in {
    import scair.dialects.dlam_de_bruijn.*
    import DlamTy.*

    val a = n(2) // DlamNatLit(2)
    val b = n(3)
    val s = nadd(a, b) // DlamNatAdd
    val p = nmul(s, n(2)) // DlamNatMul

    s shouldBe DlamNatAdd(DlamNatLit(IntData(2)), DlamNatLit(IntData(3)))
    p shouldBe DlamNatMul(s, DlamNatLit(IntData(2)))
  }

  "NatExpr equality" should "be structural" in {
    import scair.dialects.dlam_de_bruijn.*

    val a1 = DlamNatAdd(DlamNatLit(IntData(2)), DlamNatLit(IntData(3)))
    val a2 = DlamNatAdd(DlamNatLit(IntData(2)), DlamNatLit(IntData(3)))
    val a3 = DlamNatAdd(
      DlamNatLit(IntData(3)),
      DlamNatLit(IntData(2))
    ) // different tree
  ...
  "NatLit verify" should "reject negative literals" in {
    import scair.dialects.dlam_de_bruijn.*

    DlamNatLit(IntData(0)).custom_verify() shouldBe Right(())
    DlamNatLit(IntData(5)).custom_verify() shouldBe Right(())
    DlamNatLit(IntData(-1)).custom_verify().isLeft shouldBe true
  }

  "NatExpr embedding" should "work as a parameter inside other attributes" in {
    import scair.dialects.dlam_de_bruijn.*

    final case class DlamDummySized(
        arity: scair.dialects.dlam_de_bruijn.NatExpr
    ) extends scair.ir.ParametrizedAttribute
        with scair.ir.TypeAttribute:
      override def name = "dlam.dummy_sized"
      override def parameters = Seq(arity)

    val sz = DlamNatMul(
      DlamNatLit(IntData(2)),
      DlamNatAdd(DlamNatLit(IntData(3)), DlamNatLit(IntData(4)))
    )
    val t = DlamDummySized(sz)

    printed(t) should include("!dlam.nat.mul")
  }

  "A DlamVecType" should "print and embed nat exprs correctly" in {
    import DlamTy.*
    ...
    val printed_VecTy: String = printed(vecTy)
    printed_VecTy should include("!dlam.vec")
    printed_VecTy should include("!dlam.nat_lit<4>")
    printed_VecTy should include("!dlam.bvar<0>")
  }

  "DBI.shift on vec" should "shift inside the element type but not the length" in {
    import DlamTy.*, DBI.*
  ...
  "DBI.subst on vec" should "substitute inside element type" in {
    ...
    val substed = subst(0, tau, t)
    substed shouldEqual DlamVecType(len, tau)
  }

  "DBI.instantiate on a forall vector" should
    "instantiate ∀α. vec<3, α> at i32" in {
      import DlamTy.*, DBI.*
      ...
      val inst = instantiate(poly, i32Ty)
      inst shouldEqual DlamVecType(n(3), i32Ty)
    }
