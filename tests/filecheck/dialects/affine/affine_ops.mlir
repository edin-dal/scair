...
// CHECK-NEXT:    ^bb1(%4: index):
// CHECK-NEXT:      "affine.yield"() : () -> ()
// CHECK-NEXT:    }) : () -> ()
// CHECK-NEXT:    "affine.parallel"(%0) <{lowerBoundsMap = affine_map<()[] -> (0)>, lowerBoundsGroups = dense<1> : vector<[1]xi32>, upperBoundsMap = affine_map<()[s0] -> (s0)>, upperBoundsGroups = dense<1> : vector<[1]xi32>, steps = [1], reductions = []}> ({
// CHECK-NEXT:    ^bb2(%5: index):
// CHECK-NEXT:      "affine.yield"() : () -> ()
// CHECK-NEXT:    }) : (index) -> ()
// CHECK-NEXT:    "affine.store"(%1, %2) <{map = affine_map<()[] -> (0, 0)>}> : (f64, memref<2x3xf64>) -> ()
// CHECK-NEXT:    %6 = "affine.apply"(%3, %3) <{map = affine_map<(d0, d1)[] -> (d0 + d1, d1)>}> : (index, index) -> (index)
// CHECK-NEXT:    %7 = "affine.min"(%3) <{map = affine_map<(d0)[] -> (d0 + 41, d0)>}> : (index) -> (index)
// CHECK-NEXT:    %8 = "affine.load"(%2, %3, %3) <{map = affine_map<(d0, d1)[] -> (d0, d1)>}> : (memref<2x3xf64>, index, index) -> (f64)
