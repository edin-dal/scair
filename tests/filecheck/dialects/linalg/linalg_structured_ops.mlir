...
//
// One deviation from MLIR to note:
//
...
//     property whenever an op has two variadic operand groups. MLIR accepts it
//     on input and simply drops it again on output.
//
// The contraction ops (matmul, contract, batch_matmul, batch_reduce_matmul)
// carry a real multiply-accumulate body rather than a trivial yield: MLIR's
// ContractionOpInterface verifier rejects anything else with "expected add/mul
// op in the body".
...
^bb0(%a: f32, %b: f32, %c: f32):
  %p0 = "arith.mulf"(%a, %b) : (f32, f32) -> f32
  %p1 = "arith.addf"(%c, %p0) : (f32, f32) -> f32
  "linalg.yield"(%p1) : (f32) -> ()
...
// CHECK-NEXT:   ^bb0(%8: f32, %9: f32, %10: f32):
...
// CHECK-NEXT:   %14 = "linalg.elementwise"(%0, %0, %1) <{kind = #linalg.elementwise_kind<add>, operandSegmentSizes = array<i32: 2, 1>, indexing_maps = [#map, #map, #map]}> ({
...
// CHECK-NEXT:     %23 = "arith.mulf"(%20, %21) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
// CHECK-NEXT:     %24 = "arith.addf"(%22, %23) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
...
// CHECK-NEXT:     %24 = "arith.mulf"(%21, %22) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
// CHECK-NEXT:     %25 = "arith.addf"(%23, %24) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
...
// CHECK-NEXT:     %25 = "arith.mulf"(%22, %23) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
// CHECK-NEXT:     %26 = "arith.addf"(%24, %25) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
...
// CHECK-NEXT:     %29 = "arith.mulf"(%26, %27) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
// CHECK-NEXT:     %30 = "arith.addf"(%28, %29) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
...
// CHECK-NEXT:     %30 = "arith.mulf"(%27, %28) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
// CHECK-NEXT:     %31 = "arith.addf"(%29, %30) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
