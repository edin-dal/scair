...
//
//   * `iterator_types` is an array of i32 ordinals into IteratorType (0 is
//     `parallel`, 1 is `reduction`, 2 is `window`) rather than an array of
//     `#linalg.iterator_type<...>` attributes, and `kind` and `cast` are
//     likewise ordinals. ScaIR represents enums as their ordinal IntegerAttr;
//     see LinalgEnums.scala.
//
...
// CHECK-NEXT:   %3 = "linalg.generic"(%0, %1) <{iterator_types = [0 : i32, 0 : i32], indexing_maps = [#map, #map], doc = "an element-wise copy", operandSegmentSizes = array<i32: 1, 1>, library_call = "copy"}> ({
// CHECK-NEXT:   ^bb0(%4: f32, %5: f32):
// CHECK-NEXT:     linalg.yield %4 : f32
// CHECK-NEXT:   }) : (tensor<4x8xf32>, tensor<4x8xf32>) -> tensor<4x8xf32>
// CHECK-NEXT:   %4 = "linalg.generic"(%0, %1) <{indexing_maps = [#map, #map], iterator_types = [0 : i32, 1 : i32], operandSegmentSizes = array<i32: 1, 1>}> ({
// CHECK-NEXT:   ^bb0(%5: f32, %6: f32):
// CHECK-NEXT:     linalg.yield %5 : f32
// CHECK-NEXT:   }) : (tensor<4x8xf32>, tensor<4x8xf32>) -> tensor<4x8xf32>
// CHECK-NEXT:   %5, %6 = "test.op"() : () -> (memref<4x8xf32>, memref<4x8xf32>)
// CHECK-NEXT:   "linalg.generic"(%5, %6) <{indexing_maps = [#map, #map], iterator_types = [0 : i32, 0 : i32], operandSegmentSizes = array<i32: 1, 1>}> ({
