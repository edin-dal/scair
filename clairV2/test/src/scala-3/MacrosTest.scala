...
class MacrosTest extends AnyFlatSpec with BeforeAndAfter {
  ...
  "Unverified instantiation" should "Correctly instantiates the UniverifiedOp" in {
    ...
    unverOp.operands should matchPattern { case ListType(Value(IntegerType(IntData(5), Unsigned)), Value(IntegerType(IntData(5), Unsigned))) => }
    unverOp.results should matchPattern { case ListType(Result(IntegerType(IntData(25), Unsigned))) => }
    unverOp.dictionaryProperties("randProp") should matchPattern { case StringData("what") => }
  }
  ...
  "Conversion to ADTOp" should "Correctly translate from Unverified operation to ADT Operation" in {
    ...
    adtOp.lhs should matchPattern { case Value(IntegerType(IntData(5), Unsigned)) => }
    adtOp.rhs should matchPattern { case Value(IntegerType(IntData(5), Unsigned)) => }
    adtOp.result should matchPattern { case Value(IntegerType(IntData(25), Unsigned)) => }
  ...
  "ADTOp test for correctly passing around the same instances" should "test all fields of a ADTOp" in {
    ...
    adtOp.randProp1.typ equals unverOp.dictionaryProperties("randProp1") should be (true)
    adtOp.randProp2.typ equals unverOp.dictionaryProperties("randProp2") should be (true)
