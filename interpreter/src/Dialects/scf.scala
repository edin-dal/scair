...
object run_for extends OpImpl[scf.ForOp]:
  ...
  def compute(
    ...
    args match
      case Seq(lowerBound: Int, upperBound: Int, step: Int, rest*) =>
        ...
        for i <- lowerBound until upperBound by step do
          ...
          loopArgs = i +:
            loopArgs // Prepend induction variable to loop args for next iteration
      ...
      case _ =>
        throw new Exception(
          "For loop operands must be (Int, Int, Int, ...iter_args)"
        )
