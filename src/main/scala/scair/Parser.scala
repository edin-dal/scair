...
object Parser {
    ...
    class Context(var localValueMap: mutable.Map[String, Value]) {
        ...
        def testParse[A](text: String, parser: fastparse.P[_] => fastparse.P[A]): fastparse.Parsed[A] = {
    ...
    def DecimalLiteral[$: P] = P( Digit.rep(1).! ).map((literal: String) => literal.toInt)
    def HexadecimalLiteral[$: P] = P( "0x" ~~ HexDigit.rep(1).! ).map((hex: String) => Integer.parseInt(hex, 16))
    ...
    def ValueUseList[$: P] = P( ValueUse ~ ( "," ~ ValueUse ).rep ).map((valueUseList: (String, Seq[String])) => valueUseList._1 +: valueUseList._2)
    ...
    def generateOperation(operation: (Option[Seq[String]], (String, Option[Seq[String]], (Seq[Attribute], Seq[Attribute])))): Operation = {
        ...
        val resultss: Seq[Value] = for { n <- (0 to results.length - 1) } yield valueMap.contains(results(n)) match {
            case true => 
        ...
        val operandss: Seq[Value] = for {n <- {0 to operands.length - 1}} yield !valueMap.contains(operands(n)) match {
            case true => 
            ...
            case false => 
                ...
                valueMap(name).typ != operandsTypes(n) match {
                    case true =>
                        throw new Exception(s"$name use with type ${operandsTypes(n)} but defined with type ${valueMap(name).typ}")
                    case false =>
                        valueMap(name)
            }
        }
    ...
    def OpResultList[$: P] = P( OpResult ~ ( "," ~ OpResult ).rep ~ "=" ).map((results: (Seq[String], Seq[Seq[String]])) => results._1 ++ results._2.flatten)
    ...
    def Type[$: P] = P( TypeAlias | BuiltIn ).map((typeName: String) => Seq(new Attribute(name = typeName))) // shortened definition TODO: finish...

    def TypeListNoParens[$: P] = P( Type ~ ( "," ~ Type ).rep ).map((types: (Seq[Attribute], Seq[Seq[Attribute]])) => types._1 ++ types._2.flatten)
    ...
    def main(args: Array[String]): Unit = {
        ...
        println(context1.testParse(text = "%0, %1, %2 = \"test.op\"() : () -> (i32, i64, i32)", parser = TopLevel(_)))
