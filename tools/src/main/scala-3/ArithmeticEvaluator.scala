  ...
  def interpret_bin_op(
    ...
    (lval, rval) match
      case (li: IntegerAttr, ri: IntegerAttr) =>
        IntegerAttr(
          IntData(combine(li.value.value.toInt, ri.value.value.toInt)),
          li.typ
        )
      case _ =>
        throw new Exception(s"Unsupported operand types for binary operation")
  ...
  def interpret_cmp_op(
    ...
    (lval, rval) match
      case (li: IntegerAttr, ri: IntegerAttr) =>
        ...
        predicate match
          case 0 => // EQ
            if lcmp == rcmp then IntegerAttr(IntData(1), I1)
            else IntegerAttr(IntData(0), I1)
          case 1 => // NE
            if lcmp != rcmp then IntegerAttr(IntData(1), I1)
            else IntegerAttr(IntData(0), I1)
      ...
      case other =>
        throw new Exception("Unsupported operand types for cmpi, got: $other")
