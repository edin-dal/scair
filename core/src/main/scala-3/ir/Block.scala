...
object Block {
  ...
  ): Block = new Block(arguments_types, operations_expr)
...
case class Block private (
  ...
  def this(
    ...
    ListType.from((operations match {
      ...
      case multiple: Iterable[_] => multiple.asInstanceOf[Iterable[Operation]]
    }))
  )
  ...
  private def this(
    ...
    ListType.from(args._1 match {
      ...
      case multiple: Iterable[_]    => multiple
          .asInstanceOf[Iterable[Value[Attribute]]]
    }),
  ...
  def verify(): Either[String, Unit] = {
    arguments.foldLeft[Either[String, Unit]](Right(
      ()
    ))((res, arg) => res.flatMap(_ => arg.verify())).flatMap(_ =>
