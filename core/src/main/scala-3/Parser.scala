...
object Parser {
  ...
  class Scope(
    ...
    def switchWithParent: Scope = parentScope match {
      case Some(x) =>
        checkValueWaitlist()
        checkBlockWaitlist()
        x
  ...
  type ScairParsingRun[+T] = ParsingRun[T]

  type P[+T] = ScairParsingRun[T]

  extension[T] (p : ScairParsingRun[T])
    def scope : Scope = p.misc.getOrElse("scope", Scope()).asInstanceOf[Scope]
