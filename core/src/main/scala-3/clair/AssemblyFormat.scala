  ...
  def buildOperation(
    ...
    val operandNames = Map
      .from(
        parsedDirectives.zipWithIndex.flatMap((d, i) =>
          d match
            case VariableDirective(OperandDef(name = name)) => Some(name -> '{ $parsed(${ Expr(i) }) }.asExprOf[Any])
            case _                                          => None
        )
      )
    val operandNamesArg =
    ...
    val operandTypes = Map
      .from(
        parsedDirectives.zipWithIndex.flatMap((d, i) =>
          d match
            case TypeDirective(OperandDef(name = name)) => Some(name -> '{ $parsed(${ Expr(i) }) }.asExprOf[Any])
            case _                                      => None
        )
      )
    val operandTypesArg =
    ...
    val resultTypes = Map
      .from(
        parsedDirectives.zipWithIndex.flatMap((d, i) =>
          d match
            case TypeDirective(ResultDef(name = name)) => Some(name -> '{ $parsed(${ Expr(i) }) }.asExprOf[Any])
            case _                                     => None
        )
      )
    val resultTypesArg =
