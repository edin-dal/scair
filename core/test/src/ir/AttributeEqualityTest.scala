...
class AttributeEqualityTest extends AnyFlatSpec:
  ...
  "A derived attribute" should "equal a distinct instance with equal parameters" in {
  ...
  it should "differ from another class with the same parameters" in {
    assert(
  ...
  "A parameterless derived attribute" should "equal any other instance of its class" in {
  ...
  "IntegerType" should "equal a freshly built one of the same width and sign" in {
    assert(IntegerType(IntData(32), Signless) == IntegerType(IntData(32), Signless))
  ...
  it should "differ on width or on signedness" in {
    assert(IntegerType(IntData(32), Signless) != IntegerType(IntData(64), Signless))
    assert(IntegerType(IntData(32), Signless) != IntegerType(IntData(32), Signed))
    assert(IntegerType(IntData(32), Signed) != IntegerType(IntData(32), Unsigned))
  }
