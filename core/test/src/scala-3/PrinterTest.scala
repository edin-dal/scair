...
class PrinterTest extends AnyFlatSpec with BeforeAndAfter {
  ...
  "printRegion" should "return the correct string representation of a region" in {
    val region = Region(Seq(Block(ListType(UnregisteredOperation("op1")))))
    val expected = """{
                     |  "op1"() : () -> ()
                     |}""".stripMargin
  ...
  "printBlock" should "return the correct string representation of a block" in {
    val block = Block(Seq(I32), Seq(UnregisteredOperation("op1")))
  ...
  "printOperation" should
    "return the correct string representation of an operation" in {
      val operation = UnregisteredOperation(
        ...
        regions =
          Seq(Region(Seq(Block(ListType(UnregisteredOperation("op2"))))))
      )
  ...
  "printSuccessors" should
    "return the correct string representation of a operation with successors" in {
      ...
      val successorTestBlock = Block(ListType(), ListType())

      val program = UnregisteredOperation(
        "op1",
        regions = Seq(Region(Seq(
          successorTestBlock,
          Block(Seq(UnregisteredOperation(
            "test.op",
